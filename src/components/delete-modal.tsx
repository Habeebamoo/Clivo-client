  ...
  const deleteArticle = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URL}/api/article/${id}`,
        {
          method: "DELETE",
          headers: {
  ...
  return (
    <div className="fixed top-0 bottom-0 left-0 right-0 bg-black/70 z-30 flex-center">
      ...
      >
        ...
        <h1 className="font-outfit text-center mt-2 text-xl md:text-2xl">
          Delete Article
        </h1>
        <p className="font-jsl text-sm text-gray-700 mt-4 text-center">
          You are aware that this action cannot be reversed and all data will be
          lost
        </p>
        <p className="font-jsl text-sm text-gray-700 mt-2 text-center">
          Type{" "}
          <span className="text-red-600 font-inter font-bold">delete</span> to
          confirm
        </p>
        <input
          type="text"
        ...
        />
        {loading ? (
          <button className="bg-red-100 cursor-not-allowed flex-center py-4 rounded-lg w-full">
